...
const(
	db_type = "mysql"
	db_path = "admin:GNIKAVJEXRMVWMEJ@tcp(sl-eu-lon-2-portal.1.dblayer.com:17061)/linktor"
	...
	INIT_VERIFICATION = "INSERT INTO verification (nick, hashednick, realuser, email) VALUES(?,?,?,?)"
	...
	CONFIRM_NEW_USER = "UPDATE verification SET realuser = ? WHERE nick = ?"
)
...
func InitSignUp(n string) {
	...
	// initializes verification table to not verified
func InitializeVerification(n, hn string, e string){
	db, err := sql.Open(db_type, db_path)
	HandleDBError(err, "Could not Open @ InitializeVerification")
	defer db.Close()

	prep5, err := db.Prepare(INIT_VERIFICATION)
	HandleDBError(err, "Could not Prepare @ prep3 in INIT_VERIFICATION")

	res5, err := prep5.Exec(n,hn,"nope",e)
	HandleDBError(err, "Could not Exec at res3 in INIT_VERIFICATION")

	lr5, err := res5.LastInsertId()
	HandleDBError(err, "Could not get LastInsertId lr5")
	log.Println("LastInsertId for PVERIFICATION is", lr5)
}

func ConfirmNewUser(n string) {
	db, err := sql.Open(db_type,db_path)
	HandleDBError(err, "Could not open @ ConfirmNewUser")	
	defer db.Close()

	// update real user
	prep, err := db.Prepare(CONFIRM_NEW_USER)
	HandleDBError(err, "Could not Prepare at ConfirmNewUser")

	res, err := prep.Exec("yep",&n)
	HandleDBError(err, "Error occured during execution at ConfirmNewUser")

	log.Printf("Direct from DB, I have confirmed %s", n)

	lr, err := res.LastInsertId()
	HandleDBError(err, "Error occured while getting LastInsertId at CONFIRM_NEW_USER")
	log.Println("LastInsertId for CONFIRM_NEW_USER is : ", lr)
}
